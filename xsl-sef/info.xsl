...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="/">
        ...
        <xsl:result-document href="#info"> 
            <h2> Informazioni </h2>
            <div id="prog">
                <xsl:for-each select="tei:teiCorpus/tei:teiHeader/tei:fileDesc/tei:notesStmt/tei:note">
                    <p class="noteInfo">
                        <xsl:value-of select="." />
                        <br />
                    </p>
            ...
            <div id="title_info" class="info_list">
                  ...
                  <xsl:apply-templates select="tei:teiCorpus/tei:teiHeader//tei:editionStmt//tei:respStmt[(contains(tei:resp, 'Codificato da:'))]/tei:persName"/>
              </div> 

            <div id="title_info" class="info_list">
                <h3>Supervisione alla codifica:</h3>
                <xsl:apply-templates select="tei:teiCorpus/tei:teiHeader//tei:editionStmt//tei:respStmt[(contains(tei:resp, 'Supervisionato da:'))]/tei:persName"/>
            </div> 

            <div id="edition_info" class="info_list">
                  <h3>Responsabili di edizione:</h3>
                  <xsl:if test="tei:teiCorpus/tei:teiHeader//tei:editionStmt//tei:respStmt[position() > 1]">
                      <xsl:apply-templates select="tei:teiCorpus/tei:teiHeader//tei:editionStmt//tei:respStmt[not(contains(tei:resp, 'Codificato da:'))]" />
                  </xsl:if>
              </div>

              <div id="publication_info" class="info_list">
                  <h3>Pubblicazione e distribuzione:</h3>
                  <xsl:apply-templates select="tei:teiCorpus/tei:teiHeader//tei:publicationStmt"/>
              </div>

              <div id="publication_info" class="info_list">
                  <h3>Ente di appartenenza:</h3>
                  <xsl:apply-templates select="tei:teiCorpus/tei:teiHeader//tei:titleStmt/tei:respStmt/tei:name"/>
              </div>
