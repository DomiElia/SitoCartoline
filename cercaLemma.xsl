      ...
      <root-tmp>
        ...
        <xsl:text>[</xsl:text>
              <xsl:for-each select="$lemmi">
                ...
                <xsl:variable name="cartXlemma" select="$root//tei:div[@type='linguistic']/tei:ab/tei:seg[@lemma = $l]/ancestor::tei:TEI/@xml:id" />
            ...
            <xsl:text>       {   </xsl:text>
                  ...
                  <xsl:text>     "descrizione" : </xsl:text> <xsl:text>"</xsl:text><xsl:for-each select="$cartXlemma" >
                  <xsl:variable name ="cartolinaid" select="."/>
                  <img src='img-cartoline/{$cartolinaid}F.jpg' alt='{$cartolinaid}' id='{$cartolinaid}' class='imgMitt' onclick = "displayMeta(this.id)"/></xsl:for-each><xsl:text>"</xsl:text>
